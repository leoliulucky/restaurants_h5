...
<header data-am-widget="header" class="am-header am-header-default sq-head ">
    <div class="am-header-left am-header-nav">
        <a href="/user/inputMobile4Register?mobile=${data.mobile}" class="" style="border: 0;">
...
<button class="paybtn" type="button" onclick="return verifyMobileCode();">验证手机号</button>
</body>
...
function verifyMobileCode() {
    ...
    $.ajax({
        type: 'POST',
        url: "/user/verifyMobileCode4Register",
        ...
        success: function(data) {
            ...
            window.location.href = "/user/verifySuccess4Register";
        }
    });
}
function getMobileCode() {
    ...
    $.ajax({
        ...
        data: {"mobile": $mobile.val(), "mai": $mai.val()},
        dataType: 'json',
        success: function(data) {
            if(data.code < 0){
                $errorTip.html(data.msg).show();
